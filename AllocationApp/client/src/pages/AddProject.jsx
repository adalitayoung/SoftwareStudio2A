...
class addProject extends Component {
  ...
  handleAddProject = async event => {
    ...
    const { projectName, projectDesc, chooseClass, minStudents, maxStudents,} = this.state
    // Add in validation here
    if (projectName !== '' && projectDesc !== '' && minStudents !== '' && maxStudents !== '') {
      await api.createProject({ projectName, projectDesc, chooseClass, minStudents, maxStudents}).then(res => {
  ...
  handleAddRole = async event => {
    ...
    if (roleName !== '' && positionsReq !== '') {
      await api.createProjectRole({roleName, positionsReq }).then(res => {
        // Do whatever you want to do whether its a page redirect etc.
        console.log(res)
        if (res.status === 201) {
          // Success condition
              ...
              <div className="form-group">
                ...
                <div className="name">Class</div>
                  <form className="name" onSubmit={this.handleAddProject}>
                    <select id="class" className="form-control" value={this.state.chooseClass} onChange={(e)=>this.setState({chooseClass:e.target.value})}>
                ...
                <div className="students-container split-container">
                  <div>
                    ...
                    <input type="roleName" className="form-control form-role" onChange={(e)=>this.setState({roleName:e.target.value})}></input>
                  </div>
                  <div>
                    <div className="namemax">Positions Required</div>
                    <input type="positionsReq" className="form-control form-role" onChange={(e)=>this.setState({positionsReq:e.target.value})}></input>
                  </div>
                  <div style={{paddingTop: '30px',}}>
                    <button className="button button--add-role" onClick={this.handleAddRole}>Add Role</button>
                  </div>
                </div>
              </div>
              <div className="button-container">
                <button type="button" className="button button--add-project" onClick={this.handleAddProject}>Add Project</button>
              </div>
             </div>
         </div>
       </div>
